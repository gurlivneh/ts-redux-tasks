...
const DELETE_REQUEST = 'userEvnets/create_request';

interface CreateRequestAction extends Action<typeof DELETE_REQUEST> { }

const DELETE_SUCCESS = 'userEvents/create_suscces';

interface CreateSuccessAction extends Action<typeof DELETE_SUCCESS> {
	payload: {
		event: UserEvent;
	};
}

const DELETE_FAILURE = 'userEvents/create_failure';

interface CreateFailureAction extends Action<typeof DELETE_FAILURE> {
	error: string;
}

export const DeleteUserEvent = (): ThunkAction<
	void,
	RootState,
	undefined,
