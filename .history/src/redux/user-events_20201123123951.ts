...
const userEventsReducer = (state: UserEventsState = initialState, action: LoadSuccessAction | CreateSuccessAction  | DeleteSuccessAction) => {
	switch (action.type) {
		case LOAD_SUCCESS:
			...
			case DELETE_SUCCESS:
			// const index = state.allIds.findIndex(element => element.id === action.payload.event.id)
			// state.allIds.forEach(element => {
			// 	if (element.id === action.payload.event.id) {
			// 		const index = state.allIds.indexOf(element.id)
